    ...
    if (xSemaphoreTake(sharedDataMutex, portMAX_DELAY) == pdTRUE) {
        if (currentRole == HYDRO_CONTROL_GE) {
            if (msgType == "DISCOVERY") {
                ...
                if (msgType == "STATUS") {
                    Serial.print("Received STATUS from ");
                    Serial.println(managedDevices[senderId].name);
                    if (managedDevices[senderId].role == AQUA_RESERV_PRO) {
                        ...
                        Serial.println(managedDevices[senderId].isFull ? "FULL" : "EMPTY");
...
void checkFillingLogic() {
    ...
    for (auto const& [wellId, reservoirIds] : wellAssignments) {
        ...
        if (managedDevices.count(wellId)) {
            // SAFETY CHECK: Do not operate a pump that is in a fault state.
            if (managedDevices[wellId].faultActive) {
                shouldPumpBeOn = false; // Override logic if fault is active
            }
